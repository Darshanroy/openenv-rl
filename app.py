...
SAMPLE_PROMPTS = {
    ...
    "easy_payment_fail": [
        ...
        "Why did my transaction for ORD-1414 not go through?",
        "Help with payment failure.",
    ],
    "easy_coupon": [
        "I have a coupon SAVE10 but it's not working.",
        "Apply SAVE10 to my cart please.",
        "Validating SAVE10 coupon.",
    ],
    "easy_account": [
        "I forgot my password for meera.reddy@example.com.",
        "Reset my account password.",
        "Login issues with my meera email.",
    ],
    "medium_delay": [
        "My order ORD-909 is late.",
        "When will ORD-909 get here? It's delayed.",
        "Check delay on ORD-909.",
    ],
    "easy_cancel": [
        "Cancel my order ORD-505 immediately.",
        "I don't want ORD-505 anymore.",
        "Stop the delivery for ORD-505.",
    ],
    "medium_address": [
        "Change my address for ORD-1919 to '789 New Street'.",
        "Update delivery location for ORD-1919.",
        "Wrong address for ORD-1919.",
    ],
    "medium_reschedule": [
        "Can we reschedule ORD-2323?",
        "I won't be home for ORD-2323, change time.",
        "Change delivery date for ORD-2323.",
    ],
    "medium_return": [
        "I want to return ORD-2020.",
        "The items in ORD-2020 are wrong, I want a return.",
        "Requesting return for ORD-2020.",
    ],
    "medium_double_charge": [
        "I was charged twice for ORD-1515.",
        "Refund the second charge on ORD-1515.",
        "Double payment for ORD-1515.",
    ],
    "hard_refund": [
        "I need a full refund for ORD-2121.",
        "Give me my money back for ORD-2121.",
        "I want to refund ORD-2121.",
    ],
    "hard_damaged": [
        "My order ORD-2222 is damaged.",
        "ORD-2222 arrived broken in pieces.",
        "The box for ORD-2222 was crushed.",
    ],
    "hard_missing": [
        "My order ORD-1313 shows as delivered but it's not here.",
        "Missing items from ORD-1313.",
        "Where is ORD-1313? I checked everywhere.",
    ],
    "hard_angry": [
        "I'm EXTREMELY angry! ORD-909 is still missing!",
        "This is terrible service for ORD-909!",
        "Worst experience ever with ORD-909!",
    ],
    "hard_escalation": [
        "I want to speak to your manager about ORD-1414.",
        "Escalate my case for ORD-1414.",
...
- `investigate_missing(order_id)` — Opens a missing-item case.

**Returns & Refunds**
- `validate_return(order_id)` — Eligibility check.
- `ask_proof(order_id)` — Requests damage photo evidence.
- `create_return_request(order_id)` — Creates a return label.
...
The agent communicates with the database using square brackets:
`[tool_name('parameter')]`

**Rule 2 — Turn Limit**
Every interaction has a **6-turn limit**. If the agent hasn't resolved the issue by then, it is marked as failed.

**Rule 3 — Respond to the Customer**
After retrieving data from tools, the agent must call `[respond('message')]` to close the conversation.

**Rule 4 — Escalation Protocol**
For critical issues (angry customers / security), the agent is trained to use `[escalate_to_human('reason')]`.

**Rule 5 — No Hallucination**
The agent must ONLY use information returned by the tools. It cannot fabricate order IDs, statuses, or prices.
"""
...
def chat_step(message, history, task_id):
    ...
    prompt = (
        ...
        "Use tools like [get_order('...')] or respond to the user.\n"
        f"Scenario: {task_id}\n"
        f"Observation: {obs}\n"
        "Assistant:"
    )
        ...
        with gr.Column(scale=3):
            with gr.Tabs():
                ...
                with gr.Tab("💬 Agent Conversation"):
                    chatbot = gr.Chatbot(label="Simulated Chat Session", height=450, type="messages")
                    with gr.Row():
